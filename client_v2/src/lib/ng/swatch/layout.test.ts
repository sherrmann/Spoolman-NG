// Ported from the classic client's client/src/utils/swatch/layout.test.ts (#415 step 3).
...
import type { MarkRect, SwatchInput, SwatchLayout, SwatchStyleSpec } from './layout';
...
function qrCellCovered(layout: SwatchLayout, gridRow: number, gridColumn: number): boolean {
	...
	return containsPoint(
		layout.markRects,
		layout.qr.x + (gridColumn + 0.5) * size,
		layout.qr.y + (gridRow + 0.5) * size
	);
}
...
describe('buildSwatchLayout', () => {
	...
	it('shrinks slightly-long lines and truncates extreme ones at the minimum scale', () => {
		const layout = buildSwatchLayout(
			{ ...FULL_INPUT, name: 'A Truly Unreasonably Long Filament Name' },
			TEST_SPEC
		);
