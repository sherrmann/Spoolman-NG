// Ported from the classic client's client/src/utils/swatch/styles.test.ts (#415 step 3).
...
//
// Adapted for client_v2: the classic test parses the generated 3MF model XML
// with the browser's DOMParser (jsdom in the classic client's vitest config).
// client_v2's vitest runs in a plain node environment with no jsdom/DOMParser
// dependency, so this uses the local parseXml helper instead (see
// xmlTestHelpers.ts) — same assertions, different parser.

import { strFromU8, unzipSync } from 'fflate';
import { describe, expect, it } from 'vitest';
import { assertWatertight, meshBounds } from './meshHelpers';
import { buildSwatchLayout } from './layout';
...
import { parseXml } from './xmlTestHelpers';
...
describe.each(SWATCH_STYLES.map((style) => [style.key, style] as const))(
	'style contract: %s',
	(_key, style) => {
		describe.each(FIXTURES)('with %s', (_label, input) => {
			...
			it('keeps the marking clear of any hole (keychain or hanger tab)', () => {
				const holes = [
					style.spec.hole,
					style.spec.hangerTab
						? { cx: style.spec.hangerTab.cx, cy: 0, r: style.spec.hangerTab.holeR }
						: undefined
			...
			it('builds watertight meshes with the marking one layer above the base, sunk in for a robust union', () => {
				...
				expect(markingBounds.max[2]).toBeCloseTo(
					style.spec.baseThicknessMm + style.spec.markingThicknessMm,
					9
				);
			...
			it('packages as a parseable 3MF archive', () => {
				...
				const doc = parseXml(strFromU8(files['3D/3dmodel.model']));
