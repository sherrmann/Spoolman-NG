// Ported from the classic client's client/src/utils/swatch/threeMf.ts (#415 step 3).
...
export function buildSwatchMeshes(layout: SwatchLayout): SwatchMeshes {
	...
	const outline = roundedRectanglePolygon(
		layout.widthMm,
		layout.heightMm,
		layout.cornerRadiusMm,
		CORNER_SEGMENTS
	);
	if (layout.hangerTab) {
		addExtrudedPlateWithHangerTab(
...
function combinedMeshXml(base: Mesh, marking: Mesh): string {
	...
	for (let i = 0; i < base.triangles.length; i += 3) {
		parts.push(
			`<triangle v1="${base.triangles[i]}" v2="${base.triangles[i + 1]}" v3="${base.triangles[i + 2]}"/>`
		);
	}
