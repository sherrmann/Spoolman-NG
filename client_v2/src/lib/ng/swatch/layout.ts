// Ported from the classic client's client/src/utils/swatch/layout.ts (#415 step 3).
...
import { normalizeHexColor, pickMarkingColor } from './color';
import type { MarkingColor } from './color';
import {
	CHAR_ADVANCE,
	DEGREE_SIGN,
	GLYPH_HEIGHT,
...
function chooseQrModules(payload: string, qrAreaMm: number): { modules: boolean[][]; ecLevel: QrEcLevel } {
	...
	const moduleSizeOf = (modules: boolean[][]) =>
		snappedModuleSizeMm(qrAreaMm, modules.length + 2 * QR_QUIET_MODULES);
