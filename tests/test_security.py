...
def _guarded_app() -> FastAPI:
    ...
    @app.patch("/thing")
    async def _amend() -> dict[str, bool]:
        return {"ok": True}
...
def test_middleware_refuses_a_cross_origin_patch(client: TestClient):
    """PATCH is this fork's usual partial-update method, so cover it alongside POST and DELETE."""
    assert client.patch("/thing", headers={"Origin": "https://evil.example"}).status_code == 403
...
def test_a_refused_websocket_carries_no_data_before_the_close(client: TestClient):
    """The refusal happens before ``accept``, so the page never receives a single message.

    The Docker matrix asserts the same refusal as an HTTP 403; from inside the ASGI stack it
    surfaces as a disconnect carrying the close code that echoes it.
    """
    with (
        pytest.raises(WebSocketDisconnect) as refusal,
        client.websocket_connect("/thing", headers={"Origin": "https://evil.example"}) as ws,
    ):
        ws.receive_json()
    assert refusal.value.code == security.WS_CLOSE_FORBIDDEN
