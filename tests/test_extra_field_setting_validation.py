...
Ported from upstream (Donkie/Spoolman) alongside the extra_field_registry.py structural port: the
/field endpoints validate what they write, but the generic /setting/{key} endpoint did not. A
malformed extra_fields_* array written that way was accepted and then failed to parse on every
later read, leaving GET /field/{entity} permanently returning 500.

Kept as a separate file from tests/test_extra_field_registry.py, which already covers this fork's
own validate_extra_field_value/validate_extra_field/add_or_update_extra_field tests under the same
module name upstream used for this file.
"""
...
def test_entity_type_of_setting_recognises_each_entity():
    ...
    # Fork-only entities (issues #103/#75): the registry backs their real field tables/routers too.
    assert registry.entity_type_of_setting("extra_fields_location") is EntityType.location
    assert registry.entity_type_of_setting("extra_fields_printer") is EntityType.printer
...
def test_link_field_without_template_is_rejected():
    """Fork extension (#129): the link_template invariant must also apply through this path."""
    payload = json.dumps([field(field_type="link")])
    with pytest.raises(ValueError, match="index 0"):
        registry.validate_extra_field_setting("extra_fields_spool", payload)
