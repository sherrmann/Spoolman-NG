...
- Get the diff you are asked to review (for example `git diff origin/master...HEAD`, or the
  ...
  enough to judge.
- Look for, in order: correctness bugs (edge cases, error handling, concurrency, SQLite vs
  Postgres/MySQL differences, API backward compatibility under `/api/v1`), missing or weak
