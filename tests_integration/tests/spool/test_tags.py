...
This is a separate mechanism from this fork's existing NFC/RFID support (spoolman/api/v1/nfc.py,
which links a tag via the spool's `nfc_tag_uid` extra field): the two coexist without touching
each other.
"""
...
def test_many_tags_on_one_spool(random_filament: dict[str, Any]):
    """A spool can carry a vendor tag and a copy of it on a blank sticker; a real case (upstream #776)."""
...
def test_find_by_tag_combines_with_other_filters(random_filament: dict[str, Any]):
    """The tag join goes through the shared find() statement, so it composes and the count agrees."""
    uid = _uid()
