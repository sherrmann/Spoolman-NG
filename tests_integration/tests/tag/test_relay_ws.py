...
async def _prime(ws: ClientConnection) -> None:
    """Confirm the socket loop is alive before triggering the event that follows.

    The endpoint's idle loop only writes back after it has read something, so a "ping" here
    is what makes the subsequent assertion about the *next* message meaningful.
    """
    await ws.send("ping")
    check = json.loads(await asyncio.wait_for(ws.recv(), timeout=RECV_TIMEOUT_S))
...
async def test_scan_reaches_the_readers_own_pool():
    ...
    async with websockets.connect(ws_url(f"/api/v1/tag/scan/{reader_id}")) as ws:
        await _prime(ws)
...
async def test_scan_does_not_reach_another_readers_pool():
    ...
    async with websockets.connect(ws_url(f"/api/v1/tag/scan/{mine}")) as ws:
        await _prime(ws)
...
async def test_root_scan_pool_follows_every_reader():
    ...
    async with websockets.connect(ws_url("/api/v1/tag/scan")) as ws:
        await _prime(ws)
...
async def test_scans_never_reach_the_entity_websockets():
    ...
    async with (
        websockets.connect(ws_url("/api/v1/")) as root,
        websockets.connect(ws_url("/api/v1/spool")) as spools,
    ):
        await _prime(root)
        await _prime(spools)
        _scan(uid, reader_id).raise_for_status()
...
async def test_linking_a_tag_emits_a_spool_event(random_filament: dict[str, Any]):
    ...
    spool = result.json()
    try:
        async with websockets.connect(ws_url(f"/api/v1/spool/{spool['id']}")) as ws:
            await _prime(ws)
