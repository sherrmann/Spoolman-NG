"""Shared extra-field definitions and settings access."""
...
def validate_extra_field_value(field: ExtraFieldParameters, value: str) -> None:  # noqa: C901, PLR0912, PLR0915
    ...
    if field.field_type == ExtraFieldType.text:
        ...
        if len(data) != 2:  # noqa: PLR2004
            raise ValueError("Value list must have exactly two values.")
        if not all((isinstance(item, (float, int)) or item is None) and not isinstance(item, bool) for item in data):
        ...
        if field.multi_choice:
            ...
            if field.choices is not None and not all(item in field.choices for item in data):
