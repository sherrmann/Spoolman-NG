...
Spoolman NG's own token/account auth (spoolman/auth.py) is opt-in and off by default, so on
most deployments -- and on every deployment for reads, since the auth layer only ever gates
individual endpoints, never the transport -- the network is the only boundary protecting a
user's data. This module answers the two questions the guards here need, independently of
whatever auth is configured: "is this browser origin someone we trust?" and "is this a
hostname we answer to?".
...
class TrustedOriginMiddleware:
    ...
    On the (default) no-auth configuration -- no ``SPOOLMAN_API_TOKEN`` and no accounts, see
    spoolman/auth.py -- this is the *only* thing standing between a page on any website the user
    happens to visit and a Spoolman instance it can reach: a form post needs no CORS preflight,
    and a websocket handshake is exempt from CORS entirely, so either would otherwise let that
    page write to (or, over the websocket, read from) the user's inventory. Even when a token or
    account is configured this still closes the gap CSRF classically exploits -- a same-origin
    session cookie a form can ride along with -- though Spoolman's own bearer-token credential
    already can't be attached by a cross-origin form or the browser websocket API, which is why
    that configuration is materially safer without this middleware than the no-auth one is.
