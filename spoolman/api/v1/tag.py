...
class SpoolTagParameters(BaseModel):
    """Body for linking a tag. Spools and filaments take the same one (upstream: TagLinkParameters)."""
...
@router.post(
    ...
    description=(
        ...
        "Like every other endpoint, this one is subject to this instance's authentication policy "
        "(SPOOLMAN_API_TOKEN / user accounts) when one is configured: a reader agent then needs the "
        "same bearer token a machine client such as Moonraker would use. It is never gated by "
        "browser-origin trust, since hardware readers send no Origin header at all."
    ),
    responses={
        200: {"model": TagScan},
        400: {"model": Message},
