...
async def flatten_sqlalchemy_object(obj: models.Base, parent_key: str = "", sep: str = ".") -> dict[str, Any]:
    ...
    for attr in dir(obj):
        ...
        if not attr.startswith("_") and attr not in banned_attrs:
            ...
            if attr == "tags":
                # A collection, like `extra`, so it needs collapsing into a cell rather than
                # being left as a list of ORM objects: CSV and JSON would both write their repr.
                # Tags, a spool's or a filament's, have no keys to spread across columns, so
                # they become one comma-separated list of UIDs.
