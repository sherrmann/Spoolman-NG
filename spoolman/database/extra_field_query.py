...
class _JsonScalarText(FunctionElement):
    ...
    scalars (text/link/choice/datetime) - booleans and numbers are handled with numeric casts
    instead, since scalar decoding is not consistent for them across dialects.
    """
...
def _get_field_table_for_entity(entity_type: EntityType) -> type[models.Base]:
    ...
    if entity_type == EntityType.location:
        return models.LocationField
    if entity_type == EntityType.printer:
        return models.PrinterField
...
def _get_entity_id_column(field_table: type[models.Base]) -> InstrumentedAttribute[int]:
    ...
    if field_table == models.LocationField:
        return models.LocationField.location_id
    if field_table == models.PrinterField:
        return models.PrinterField.printer_id
...
def add_where_clause_extra_field(  # noqa: C901, PLR0912, PLR0915
    ...
    *,
    ...
    for value_part in value.split(","):
        ...
        if field_type in (ExtraFieldType.text, ExtraFieldType.link):
            ...
            # `link` (#129) stores a short per-item value with the same text-like shape, so it is
...
async def find_extra_field_values(
    *,
    ...
    Intended for text/link/single-choice/datetime fields, whose values are stored as JSON string
...
def add_order_by_extra_field(
    ...
    if field_type == ExtraFieldType.integer:
        ...
        # text, link, choice, boolean and datetime all sort on the raw stored (JSON-encoded)
        # value; link and text share the same plain-string shape so no special case is needed.
        sort_expr = value_subq

    # An entity that has no value for the field yields NULL here, and "not filled in" belongs at
