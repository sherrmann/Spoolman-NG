...
The spool entry points keep this fork's original names (`link`, `unlink`,
`find_spool_by_uid`, `try_link`), which its NFC lookup modules call; upstream renamed them
`link_spool` and so on when it added filament tags (6520ce6d), and both spellings exist here.
"""
...
async def _changed(db: AsyncSession, target: Target) -> None:
    """Emit the target's ordinary `updated` event; its tags are part of its payload.

    A filament's tags are also part of every one of its spools' payloads, since a spool embeds
    its filament, so those spools are re-announced too, as every other filament change does
    (#130).
    """
    if isinstance(target, models.Spool):
        ...
        await spool.notify_spools_of_filament_change(db, target.id)
...
# This fork's name for link_spool, which its NFC lookup modules call.
link = link_spool
...
async def try_link(
    *,
    ...
    claimed by a different spool or by a filament (a stale scan, a losing race, a tag that
    identifies a filament type) must not turn a successful read into a failed one, so the
    conflict is swallowed and logged rather than raised.
...
# This fork's name for unlink_spool.
unlink = unlink_spool
