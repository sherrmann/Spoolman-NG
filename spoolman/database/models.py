...
class Spool(Base):
    ...
    # selectin, not joined like `extra` above: the spool listing already joins filament,
    # vendor and printer, and a second joined *collection* multiplies against the
    # extra-field rows -- a spool with 5 extra fields and 3 tags would fetch 15 rows
    # instead of 5. Upstream keeps both collections on selectin for this reason; here
    # only `tags` needs it, because `extra` predates the second collection and is the
    # one the hot use/measure path is tuned around.
...
class Tag(Base):
    ...
    *_lookup.py modules), which binds a tag to a spool via the `nfc_tag_uid` extra field
    ...
    physical tags with two different UIDs (#776 upstream).
    ...
    * `target_value` -- targets that are not rows at all. A location in Spoolman is a
    ...
    spool: Mapped[Optional["Spool"]] = relationship(back_populates="tags")
    # No ORM relationship yet, deliberately. Nothing writes filament tags, and a
    # collection on Filament would add a query to every filament listing for rows that
    # cannot exist. ON DELETE CASCADE keeps the database honest in the meantime; the
    # relationship arrives with the feature that needs it.
