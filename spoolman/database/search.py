...
async def _search_spools(
    ...
    *,
    ...
    # Fork addition: Spool.printer (#75) has no upstream equivalent, but Spool.from_db reads it
    # unconditionally, so it must be eager-loaded here too or the async session lazy-loads it and
    # raises MissingGreenlet -- same reason spool.find() joinedloads it (spoolman/database/spool.py).
    load = (
...
async def _color_matches(db: AsyncSession, color_hex: str, threshold: float, limit: int) -> list[FilamentMatch]:
    ...
    Fork adaptation: upstream's find_by_color returns bare ids (id/color_hex/multi_color_hexes
    only, no vendor join), so upstream re-fetches full rows via a second find(db, ids=...) call.
    The fork's pre-existing find_by_color (unrelated to this port -- it predates it) already
    returns fully-loaded models.Filament rows with vendor eager-loaded, so that second fetch
    would misuse Filament objects as an `ids` filter. Use the rows it returns directly instead.
    """
    ...
    matched = sorted(matched, key=lambda f: _filament_color_distance(f, query_lab))
    ...
    *,
    ...
    if q.isdigit():
        ...
        spool = await db.get(
            models.Spool,
            spool_id,
            # See the comment on the same joinedload in _search_spools above.
            options=[
                joinedload(models.Spool.filament).joinedload(models.Filament.vendor),
                joinedload(models.Spool.printer),
            ],
