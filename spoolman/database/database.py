...
# bodyless cross-origin form post to /backup worth defending against (see spoolman/security.py).
# Cheap second layer behind the origin guard: refuse to rotate more often than this, and hand back
# the newest backup instead. Ported from upstream (#backup rotation hardening).
...
class Database:
    ...
    _last_rotation: float | None

    def __init__(self, connection_url: URL) -> None:
        """Construct the Database wrapper and set config parameters."""
        self.connection_url = connection_url
        # monotonic() timestamp of the last backup that actually rotated, for
        # MIN_SECONDS_BETWEEN_ROTATIONS below. None means "never rotated by this process".
