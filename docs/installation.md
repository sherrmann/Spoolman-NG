...
| `SPOOLMAN_LEGACY_CLIENT` | `TRUE` | Which web client new visitors get: `TRUE` for the classic React client, `FALSE` for the newer Svelte one. Both are built into the Docker image. The React client still covers far more of the app, so it is the default here; upstream Spoolman uses the same variable with the same meaning but the opposite default. Unless the switcher below is off, this is only the starting point — a visitor who picks the other client keeps it. |
| `SPOOLMAN_UI_SWITCHER` | `TRUE` | Whether visitors can switch between the two web clients from inside the UI. The choice is stored in a cookie, so it is per browser rather than per instance, and every URL (including printed QR labels) stays the same either way. Set to `FALSE` to hide the control and put everyone back on the `SPOOLMAN_LEGACY_CLIENT` choice, ignoring any cookie already stored. Has no effect on an install from source that built only one of the two clients. |
