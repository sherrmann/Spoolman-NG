...
| `src/lib/components/library/FilamentInspector.svelte` | One icon-button linking to `/calibration?filament=<id>`; and one import and `<FilamentCatalogueFields {filament} />` after the article-number field (#415) | Calibration belongs to a filament, and this panel is where a filament is looked at. React reaches it as a tab on `/filament/show/:id`; this client has no such page — that route is a redirect and this inspector has no tab strip. The catalogue fields are rows of the specs grid, so they have to be rendered inside it; the component emits whole `<Field>` rows and saves through a debounced saver of its own, keyed by field, since the panel's merges a patch one level deep and would let a live update undo a quick first edit |
