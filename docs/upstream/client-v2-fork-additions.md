...
| `client_v2/src/routes/+error.svelte` | The error page -- a failed `load`, an explicit `error()` or an unknown URL. Upstream has never had a `+error.svelte`, so the path is greenfield |
...
| `src/lib/components/labels/LabelDesigner.svelte` | One extra `fields.ensure('location')` beside the other three, and `location: fields.get('location')` in the `getPlaceholderGroups` call | A location label's palette has to offer location extra fields, and this component is where the palette is built. Since `EntityType` gained `location` (see the `api/fields.ts` row) the shared store answers for it, so the earlier fork block that fetched them itself is gone |
| `src/lib/components/labels/PrintLayoutPanel.svelte` | A third subject-selection path beside the spool and filament ones; and the pre-print checklist gate (#296) -- the old `doPrint` body becomes `runPrint()`, the button now opens `<PrePrintChecklist>` unless the user has opted out, and "Print now" runs the print; and `loadPage` sets `loading` on every page change and clears the rows when a page fails, so the previous page's spools are never offered under the new page's number (upstream's paging, a4c749ad, set it only once) | Each kind's picker is written out in this one component. The checklist has the same problem: the print button, `printLabels()` and therefore `window.print()` all live here, and the checklist is only worth anything immediately before that call -- it is about the settings the browser's own dialog is a moment away from applying. The dialog, the opt-out flag and the page-size maths are all fork-owned under `src/lib/ng/` |
...
| `src/lib/components/QrScannerModal.svelte` | A scanned location goes to `/location/show/<id>`; each decode is offered to `$lib/ng/components/ScanExtras` before upstream's own handling; and `setInversionMode('both')` so white-on-black labels decode where no built-in barcode detector exists (Firefox; upstream issue 1165, unfixed upstream as of `058490a3`) | The scanner has one decode callback. The delegation is three lines and deliberately *declines* an ordinary entity scan, so upstream keeps owning navigation and keeps owning it after it changes The inversion mode is set on the scanner instance the component creates, so there is no seam for it outside the component |
...
| `src/lib/library/viewPrefs.ts`, `src/lib/library/params.ts` (and their two test files) | The remembered view carries the page size (`size` in the stored entry, `pageSize: number \| null` on `StoredView`); `size` joins `VIEW_PARAMS`; `rememberedViewHref` restores it and resets the page number when it changes the size; `navigate` records it (upstream issues 1145 and 1154, open upstream as of `058490a3`) | The remembered view is one stored entry, written in `navigate` and read back in `rememberedViewHref`; a second entry just for the size would need its own redirect and could disagree with the first about which URL to land on. An entry written before the field existed reads as "no size remembered", so nobody loses a stored grouping |
| `src/lib/utils/library.ts` | An `id` entry in `FIXED_SORTS`, and `id` in `ASC_DEFAULT_SORTS` (upstream issue 1154) | `FIXED_SORTS` is the single list the sort menu and the query builders both read. The backend half is `parse_sort` letting a field's first mention win, since the query builders always append `id:asc` as a tie-breaker |
...
| `src/lib/components/library/ListToolbar.svelte` | `<LibraryModeControls />` on the line after `<NlSearchButton />`, and its import (#412) | The "Select" chip belongs with the other controls that shape the list, and the hunk is already this fork's, so it adds no new conflict site. Unmounting with the toolbar is also what drops a selection when the user leaves the Library |
| `src/lib/components/library/FilamentList.svelte` | Three import lines: `SpoolRow` from `$lib/ng/components/library/SpoolRow.svelte`, `Pagination` from `$lib/ng/components/library/LibraryFooter.svelte` (#412), and `ListToolbar` from `$lib/ng/components/library/LibraryToolbar.svelte` (#412 step 5: upstream's toolbar, then the column header, which has to sit between the toolbar and the scrolling list) | The row and the pagination are instantiated inside upstream's list components, and swapping the import is the smallest seam that reaches every instantiation without touching a template line. Both fork components render upstream's own, with every prop forwarded, so upstream can add or rename props freely. With selection off the row renders upstream's DOM exactly, which `tests_frontend_v2` (it finds rows by their link role) depends on. On a conflict in the import block: keep both sides, keep the fork's path |
| `src/lib/components/library/GroupRow.svelte`, `src/lib/components/library/UnusedRow.svelte` | One import line each: `SpoolRow` from the fork's path (#412) | As above. If upstream adds a fifth place a spool row is rendered, the symptom is rows without a checkbox there, and the fix is the same one-line import |
| `src/lib/components/library/SpoolInspector.svelte` | `openAdjust()` re-reads the stored adjust mode each time the panel opens (#412 step 2) | The mode is shared with the fork's weigh-in through the inspector's own `localStorage` key, and the inspector read it once when mounted, so a mode changed in the weigh-in never reached an inspector already on screen. The panel's state is local to this component; there is no seam to refresh it from outside |
...
| `src/lib/api/http.ts` | Every wrapper's headers go through one local `headers()` helper that merges `authHeaders()`, and a 401 hands the whole `Response` to `./auth` rather than just its status; and `putJson` takes an optional third argument of extra headers (#412), which the weigh-in uses to send an `Idempotency-Key` | This fork's backend answers 401 and upstream's does not (#406). There is no way to attach a credential to requests from outside the module that makes them, and one helper is a smaller footprint than seven inline spreads |
...
| `src/lib/components/library/ListToolbar.svelte` | `DATE_FILTERS` removed from `filterCategories`, so the three date-range categories are not offered | This fork's `GET /spool` does not implement `first_used`/`last_used`/`registered` range filtering (#409), and FastAPI drops the unknown query parameters silently -- the control returned an unnarrowed list and looked broken. The categories are built in this one array; there is nowhere else to omit them from |
| `src/lib/components/library/SpoolInspector.svelte` | One import and one `<WeightHistoryChart spoolId={spool.id} />`, between the remaining-weight gauge and the field grid | The chart reads a spool's own weigh-ins, so it belongs under the gauge those weigh-ins move — and this panel, like the filament one, is flat with no tab strip to hang a second view off. It renders nothing at all until a spool has been weighed twice, so the panel is unchanged for most spools. The chart, its maths and its fetch are all fork-owned under `src/lib/ng/` |
| `src/lib/api/fields.ts` | `'location'` and `'printer'` in `EntityType`; `link` in `FieldType`; `link_template` and `copy_from_filament` on `FieldParams`; the matching `FIELD_TYPE_LABELS` entry | This is upstream's single definition of what an extra field is. This fork's backend registers a `location` entity and a `link` field type and accepts two more definition parameters, and there is no second place to say so -- every store, manager and input reads these types. Both `Record<>` tables force their own new entry, so the build catches a half-done widening |
| `src/lib/stores/fields.svelte.ts` | `'location'` and `'printer'` in `ENTITIES`, and matching slots in the `defs` initialiser | The definitions cache is one module and is keyed `Record<EntityType, …>`; an entity the store cannot hold is an entity nothing can render. This is what let the label designer's own location fetch go away |
| `src/lib/settings/params.ts` | `'location'` and `'printer'` in `ENTITIES` | The list is the allowlist `entityFromUrl` validates `?fields=` against, so without it a link to `?fields=location` resolves to null and silently lands on the spool tab |
...
| `src/lib/components/settings/ExtraFieldsManager.svelte` | Location and Printer tabs; the `link` type label; a `link_template` input shown only for a link field (required, max 512) and a `copy_from_filament` checkbox shown only for spool fields; `onTypeChange` clears the template when leaving `link`; `save()` sends both keys, null where they do not apply; the row's type cell annotates both | This is the only place in the client where an extra field can be defined, so a parameter absent from it cannot be set at all. Neither control earns a page of its own -- each is one field on one definition -- and neither gets a table column: both are annotations on the type they qualify. Its two labels ("Link URL", "Copy from Filament") were checked against every `getByLabel` in `tests_frontend_v2` first, per the settings-page hazard noted above |
| `src/lib/components/library/FilamentInspector.svelte` | One icon-button linking to `/calibration?filament=<id>`; one import and `<FilamentCatalogueFields {filament} />` after the article-number field (#415 step 1); one import and `<FilamentImageSection {filament} />` under the manufacturer section (#415 step 2); and one import and `<FilamentSwatchButton {filament} />` beside the calibration link (#415 step 3) | Calibration belongs to a filament, and this panel is where a filament is looked at. React reaches it as a tab on `/filament/show/:id`; this client has no such page — that route is a redirect and this inspector has no tab strip. The catalogue fields are rows of the specs grid, so they have to be rendered inside it; the component emits whole `<Field>` rows and saves through a debounced saver of its own, keyed by field, since the panel's merges a patch one level deep and would let a live update undo a quick first edit. The photo sits in the right-hand column because that column is about what the filament is, and it saves through its own endpoints, so it needs nothing from the panel |
...
| `src/lib/types.ts` | `ng?: FilamentNg` on `Filament`, with its type-only import (#415) | This is the filament view model every cache, store and component shares; a field anywhere else is not carried by live updates or the saver. One optional property under a fork-owned name, so an upstream field can never collide with it |
| `src/lib/api/map.ts` | `ng: mapFilamentNg(f)` in `mapFilament`, and `Object.assign(out, filamentNgPatchToApi(patch))` in `filamentPatchToApi`, plus the import (#415) | These are the two functions every filament read and write goes through. Each hands over to `src/lib/ng/filamentCatalogue.ts` in one line, which holds the mapping and its tests |
| `src/lib/api/spoolSource.ts` | `...catalogueFromExternal(ext)` in `importExternalFilament`'s create body, plus the import (#415) | That body is built field by field here, and it left SpoolmanDB's catalogue fields out, so an imported filament lost them. The React client copies them |
| `src/lib/api/spoolSource.ts` | `ng?` on `NewFilamentDraft`, and `...catalogueForCreate(draft.ng)` in `createFilament`'s body (#415 follow-up) | The new-filament form's create body is built field by field here too; without it, catalogue fields chosen on the form or copied by a duplicate were dropped. Unknown values are left out of the body |
| `src/lib/filament/draft.ts` | `ng?` on `FilamentDraft`; `ng: catalogueFieldsOf(f.ng)` in `filamentDraftFrom`; `ng: draft.ng` in `toNewFilamentDraft`; plus the import (#415 follow-up) | This file is the new-filament form as data, shared by the add-spools flow and the change-filament dialog, and `filamentDraftFrom` is the one place a duplicate is copied. The React client's clone carries the catalogue fields, so the copy has to start with them |
| `src/lib/components/NewFilamentCards.svelte` | One import and `<NewFilamentCatalogueFields bind:draft />` after the Advanced specs grid (#415 follow-up) | The form has no slot, and the fields belong with the other specs of the filament being described. The five selects, their grid and styles are fork-owned |
