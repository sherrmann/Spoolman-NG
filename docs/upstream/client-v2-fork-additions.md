...
| `src/lib/api/fields.ts` | `'location'` in `EntityType`; `link` in `FieldType`; `link_template` and `copy_from_filament` on `FieldParams`; the matching `FIELD_TYPE_LABELS` entry | This is upstream's single definition of what an extra field is. This fork's backend registers a `location` entity and a `link` field type and accepts two more definition parameters, and there is no second place to say so -- every store, manager and input reads these types. Both `Record<>` tables force their own new entry, so the build catches a half-done widening |
| `src/lib/stores/fields.svelte.ts` | `'location'` in `ENTITIES` and a `location: []` slot in the `defs` initialiser | The definitions cache is one module and is keyed `Record<EntityType, …>`; an entity the store cannot hold is an entity nothing can render. This is what let the label designer's own location fetch go away |
| `src/lib/settings/params.ts` | `'location'` in `ENTITIES` | The list is the allowlist `entityFromUrl` validates `?fields=` against, so without it a link to `?fields=location` resolves to null and silently lands on the spool tab |
